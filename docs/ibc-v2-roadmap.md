...
Escopo: inventário funcional, matriz completa de lacunas e progresso dos lotes de implementação.

## Progresso de implementação

Os lotes M0.1, M0.2, M0.3, M1.1a e M1.1b foram implementados na branch `Pantani/cx/m0-baseline`. Além da fundação protocol-neutral e da ingestão ABCI lossless de M1.1a, M1.1b migrou o grafo raiz para Go 1.25.9, Cosmos SDK 0.54.3, CometBFT 0.39.3, IBC-Go v11.2.0, Store v2 e Log v2, preservando os contratos Classic necessários.

O sublote M1.1b-d também migrou o módulo de integração para o framework oficial `github.com/cosmos/interchaintest/v11` no pseudo-version imutável `v11.0.0-20260507171724-1a8c536981a8`, restaurou raiz e integração no `go.work`, adicionou um gate readonly isolado/workspace e adaptou a interface do relayer. Um E2E Classic real Gaia v14.1.0 + Osmosis v22.0.0 passou os nove subtestes de setup, inclusive clients, connections e channels. Isso prova compatibilidade legada do harness, não runtime IBC v11.2 nem relay IBC v2 operacional: a imagem `simd:v11.2.0` ainda precisa ser construída do source oficial e a fatia `query -> proof -> message -> broadcast` v2 continua aberta. A dívida global caiu para 138 funções com alguma complexidade `>=10` (83 ciclomáticas, 134 cognitivas; máximos 48/99), enquanto toda função criada ou tocada permanece `<10/<10`.
...
## Inventário do snapshot inicial e estado M0.1-M0.3
...
| Dynamic fee | No snapshot inicial, a query interpretava o envelope protobuf como decimal e o teste acessava RPC público; M0.1 substituiu ambos por decode tipado e fixture offline | `relayer/chains/cosmos/fee_market.go`; `fee_market_test.go` | Unit test determinístico passa; uma integração de rede deve existir apenas como suíte opt-in |
...
|---|---|---|---|---|---|---|---|---|
| Toolchain e módulos v11 | ibc-go v11.2.0; SDK v0.54.3 | M1.1b concluiu Go 1.25.9, `/v11`, Log v2, Store v2 e Comet 0.39.3; M1.1b-d compilou o framework oficial `/v11` isolado e no workspace | Construir chain runtime v11.2.0 e executar a matriz Classic v11 antes de declarar interoperabilidade | build/CI/provider | Imagem reproduzível v11.2 | `go build/test`, module verify e E2E v11.2 | P0 parcial | M0-M1 |
| Gate ciclomatico+cognitivo `<10` | contrato do projeto; baseline 2026-07-15 | M0.1: 158 violações; M0.3: 144; M1.1b-d: 83/134, união 138; M1.1b-e: 83 ciclomáticas, 130 cognitivas, união 134, máximos 48/99 | Continuar redução com gate obrigatório sem baseline/nolint | CI/todos | Modelo v1/v2 definido antes dos hotspots | zero saída de `gocyclo -over 9` e `gocognit -test -over 9` | P0 | M0-M2 |
| Modelo interno neutro | IBC v2 spec + Classic | M0.3 adicionou `Protocol`, capabilities, identificadores e envelopes de packet/event/proof/message; adaptadores Classic e v2 contract-only | Ligar o modelo às fatias verticais sem substituir o runtime Classic implicitamente | provider/core | Toolchain | Contract tests dos dois adaptadores passam | P0 concluído | M0 |
...
| Dynamic fee tipado/determinístico | SDK/Osmosis API atual | M0.1 decodifica resposta protobuf tipada, diferencia erros e usa fixture offline | Manter teste de rede somente opt-in e validar capability/version durante o upgrade SDK | Cosmos provider | SDK alvo | Unit fixture PASS; integração opt-in pendente; sem rede no `go test ./...` | P0 concluído | M0 |
| Config de path por protocolo | IBC v2 Counterparty | M0.3 adicionou `protocol: classic|v2`, client pair/prefix e validações mutuamente exclusivas; legado sem campo continua Classic | Adicionar prontidão bilateral, allowlist e opções operacionais depois das queries v11 | config/CLI | Modelo neutro | Round-trip YAML/JSON, combinações inválidas antes de RPC e guardas de runtime passam | P0 concluído | M0 |
...
| Ingestão dos eventos v2 | channel/v2 v11.2 | M1.1a reconhece os cinco eventos, classifica o contrato completo e produz sidecar neutro | Conectar o sidecar somente depois de queries/proofs/state machine v2; ainda sem relay operacional | events/parser | Tipos v2 | Goldens oficiais dos cinco eventos, Classic misto e zero fallthrough passam | P0 parcial | M1 |
| Preservação de `message.action` | docs oficiais de relayer | M1.1a preserva ordem/duplicatas/`Index`, faz join por `msg_index` e fallback legado precedente com poisoning seguro | Propagar correlation id aos consumidores v2 quando existirem | events/parser | Ingestão v2 | Multi-msg, keeper module-only, conflitos e poisoning indexado/legado passam | P0 concluído na ingestão | M1 |
| Decode seguro de packet/ack hex | eventos v2 | M1.1a usa wire local v11.2.0, caps operacionais de 512 KiB, limite de payload 256 KiB e erros tipados | Remover o wire transitório após migração completa `/v11`; validar encoding/version por app no lote correspondente | parser/security | Ingestão v2 | Golden packet/ack, truncado, oversized, invalid proto, mismatch e fuzz passam | P0 concluído na ingestão | M1 |
...
| Fixtures determinísticas de eventos | protos v11.2 | M1.1a fixa bytes/SHA oficiais de packet e ack e corpus raw multi-msg/erros sem rede | Adicionar block/tx reais e fixtures de proofs/responses quando o provider v11 existir | tests | Parser v2 | 118 testes focados com race + dois fuzz targets passam | P0 parcial | M1 |
...
- O teste de dynamic fee usa response protobuf tipada e fixture determinística. **Concluído no M0.1.**
- Existe um modelo interno protocol-neutral com adaptadores Classic/v2 e config schema versionado. **Concluído no M0.3; ainda não conectado ao loop v2.**
...
1. Gate/toolchain, fixtures, testes gigantes e codecs isolados. **Concluído em M0.1/M0.2.**
2. Config/CLI builders Classic e modelo protocol-neutral concluídos em M0.2/M0.3.
3. Adaptador de eventos v2 action-indexed concluído em M1.1a; grafo SDK/Comet/IBC e framework de integração oficial migrados em M1.1b/M1.1b-d. O próximo lote constrói a imagem v11.2 reproduzível e abre a primeira fatia vertical query/proof/msg v2.
...
- **Test truth:** no snapshot inicial, `TestQueryBaseFee` falhava por depender de RPC público. No M0.1, `go build -mod=readonly ./...` e `go test -mod=readonly ./...` passam com Go 1.25.9 e o teste usa fixture offline; testes públicos continuam proibidos como gate unitário.
