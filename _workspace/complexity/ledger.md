...
|---|---|---|---|---|---|---|---|---|---|---|---|
| cli-version | `Pantani/cx/complexity-cli-version` / `origin/main@4c1029d` | pending publication | `cmd/version.go`, `cmd/version_test.go` plus harness/state | `getVersionCmd` | `7/16` -> `5/9`; helper `3/3` | `79/122/126` -> `79/121/125`; max `48/99` unchanged | 7 tests / 14 cases pass original and refactor with race; package `cmd` 83 pass | local scores, focal race, package, lint, readonly build, diff-check, inventory PASS; global `make complexity` remains expected FAIL with 125 | APPROVED; CodeRabbit major rejected with contract evidence | no live PR collision | READY_TO_PUBLISH |

## Progress table

| PR | Files | Scores before/after | Global before/after | Gates | State |
|---|---|---|---|---|---|
| pending publication | `cmd/version.go`, `cmd/version_test.go` plus harness/state | `getVersionCmd 7/16` -> `5/9`; helper `3/3` | `79/122/126` -> `79/121/125` | PASS (intermediate global complexity expected red) | APPROVED / ready to publish |
