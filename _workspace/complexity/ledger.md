...
| cli-start | `Pantani/cx/complexity-cli-start` / `Pantani/cx/complexity-cli-config@ee1d834` | pending | `cmd/start.go`, `cmd/flags.go`, characterization/state | `startCmd`, `setupDebugServer`, `getAddInputs` | `17/33, 12/13, 11/10` -> pending | `62/102/105` -> pending; max `48/99` | 20 added scenarios; 47 focused normal/race checks and package `cmd` 138 pass on original | pending | pending | no collision with live #9/#10/#11 | REFACTORING |
